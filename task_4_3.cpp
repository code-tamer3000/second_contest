...
int64_t Merge(std::vector<int64_t>& array, int left_bound, int median, int right_bound) {
    ...
    while (i < median && j < right_bound) {
        if (array[i] < array[j]) {
            result[i - left_bound + j - median] = array[i]; i++; 
        } else {
            if (array[j] < array[i]) {  
                answer += median - i;  
            }
            result[i - left_bound + j - median] = array[j]; j++;
        }
    }
    for (; i < median; ++i) {
...
void MergeSort(std::vector<int64_t>& array, int left_bound, int right_bound, int64_t& answer) {
    if (right_bound - left_bound <= 1) {
        return;
    } 
    int median = (right_bound + left_bound) / 2;
    MergeSort(array, left_bound, median, answer);
    MergeSort(array, median, right_bound, answer);
    answer += Merge(array, left_bound, median, right_bound);
}

int64_t MergeSort(std::vector<int64_t>& array) {
    int64_t answer = 0;
    MergeSort(array, 0, array.size(), answer);
    return answer;
}
